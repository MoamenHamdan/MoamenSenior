        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                dashboardVM.RecentTransactions = await _context.Transactions
                    ...
                    .ThenInclude(tp => tp != null ? tp.ThpsThp : null)
                    .Include(t => t.TsTst)
                    .Include(t => t.TsTss)
                ...
                dashboardVM.TopSellingItems = await _context.TransactionItems
                    .Include(ti => ti.TsiIt)
                    .Where(ti => ti.TsiIt != null && !string.IsNullOrEmpty(ti.TsiIt.ItDescriptionLan1))
                ...
                dashboardVM.TopCustomers = await _context.Transactions
                    .Include(t => t.TsThpsIdBillNavigation)
                    .ThenInclude(tp => tp != null ? tp.ThpsThp : null)
                    .Where(t => t.TsThpsIdBillNavigation != null && 
                                t.TsThpsIdBillNavigation.ThpsThp != null && 
                                !string.IsNullOrEmpty(t.TsThpsIdBillNavigation.ThpsThp.ThpNameLan1) &&
                                t.TsTotalFinal.HasValue)
                ...
                dashboardVM.InventorySummary = await _context.ItemWarehouses
                    ...
                    .Where(iw => iw.ItwPlIdWhsNavigation != null && 
                                !string.IsNullOrEmpty(iw.ItwPlIdWhsNavigation.PlDescriptionLan1))
                    .GroupBy(iw => iw.ItwPlIdWhsNavigation.PlDescriptionLan1)
                    .Select(g => new WarehouseInventoryViewModel
                    {
                        WarehouseName = g.Key ?? "Unknown",
                        ItemCount = g.Count(),
                        TotalQuantity = g.Sum(iw => iw.ItwQuantity ?? 0)
                    })
                    .ToListAsync();
                ...
                dashboardVM.TransactionStatusDistribution = await _context.Transactions
                    .Include(t => t.TsTss)
                    .Where(t => t.TsTss != null && !string.IsNullOrEmpty(t.TsTss.TssDescriptionLan1))
                ...
                dashboardVM.MonthlySalesTrend = salesTrend ?? new List<MonthlySalesViewModel>();
